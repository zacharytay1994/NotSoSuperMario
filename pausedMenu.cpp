...
void pausedMenu::Initialize(Graphics& gfx, Input* input)
{
	...
	pausedMenuBGForTesting.Initialize(gfx);
...
void pausedMenu::ChildRender(bool is_testing_)
{
	if (is_testing_) { pausedMenuBGForTesting.Draw(); }
	else { pausedMenuBG.Draw(); }
...
void pausedMenu::showMenu()
{
	...
	pausedMenuBGForTesting.GetImage().setX(x);
	pausedMenuBGForTesting.GetImage().setY(y);
